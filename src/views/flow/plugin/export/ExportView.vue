...
onMounted(() => {
    ...
    // 数据导出
    console.log("画布的数据", graph.size, graph.css);
})

const onExport = ()=> {
    const graphCss = graph.css.options;
    const graphWidth = graphCss.width;
    const graphHeight = graphCss.height;
    const bg = graphCss.background.color;
    console.log(graphWidth, graphHeight, bg)

    graph.exportPNG('导出流程图', {
        height: graphHeight,
        width: graphWidth,
        backgroundColor:bg
    })
